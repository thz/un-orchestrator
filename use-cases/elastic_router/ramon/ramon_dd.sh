...
xterm -T ${name}_dd -e "${RATEMON_CLIENT} -k /etc/doubledecker/public-keys.json ${name} a ${NO_HELLO} -p ${ramon_port} -q ${config_port}  --ramon_path ${RAMON_PATH} --ramon_args -i ${interface} -s 20 -e 1 -k 10 -m 1 -a 90 -o 90 -q ${config_port}; read" &
