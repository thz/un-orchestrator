...
from iMindsAlg_Core import iMindsAlgorithm
...
def iMindsMAP(request, network):
  """
  :param request: request to be mapped
  :type request: NFFG
  :param network: the infrastructure network
  :type network: NFFG
  ...
  # average of the appropriate given parameters 
  ...
  # create the class of the algorithm
  alg = iMindsAlgorithm(network, request, edgereqdict)

  mappedNFFG,mappedNFs = alg.start()
  ...
  for req in edgereqlist:
    ...
    if srcnode.type == 'SAP':
      srcnode = mappedNFFG.network.node[
        alg.getIdOfChainEnd_fromNetwork(req.src.node.id,mappedNFs)]
      srcportid = alg._addSAPportIfNeeded(mappedNFFG, srcnode.id, srcportid)
    if dstnode.type == 'SAP':
      dstnode = mappedNFFG.network.node[
        alg.getIdOfChainEnd_fromNetwork(req.dst.node.id,mappedNFs)]
...
if __name__ == '__main__':
  ...
  sap1 = nffg.add_sap(id="SG_SAP1")
  sap2 = nffg.add_sap(id="SG_SAP2")
  sap3 = nffg.add_sap(id="SG_SAP3")
  sap4 = nffg.add_sap(id="SG_SAP4")
  nf1 = nffg.add_nf(id="er",func_type="router")
 

  nffg.add_sglink(sap1.add_port(1), nf1.add_port(1), id="hop1")
  nffg.add_sglink(sap2.add_port(1), nf1.add_port(2), id="hop2")
  nffg.add_sglink(sap3.add_port(1), nf1.add_port(3), id="hop3")
  nffg.add_sglink(sap4.add_port(1), nf1.add_port(4), id="hop4")

  nffg.add_sglink(nf1.ports[1], sap1.ports[1], id="hop11")
  nffg.add_sglink(nf1.ports[2], sap2.ports[1], id="hop22")
  nffg.add_sglink(nf1.ports[3], sap3.ports[1], id="hop33")
  nffg.add_sglink(nf1.ports[4], sap4.ports[1], id="hop44")


  with open('../examples/er-topo.nffg', "r") as g:
    net = NFFG.parse(g.read())

  mapped = iMindsMAP(nffg, net)
  print mapped.dump()
